...
class TusCreateMixin(mixins.CreateModelMixin):
    def create(self, request, *args, **kwargs):
        ...
        max_file_size = getattr(
            self, "max_file_size", tus_settings.TUS_MAX_FILE_SIZE
        )
        ...
        upload_metadata = getattr(
            request, constants.UPLOAD_METADATA_FIELD_NAME, {}
        )
    ...
    def get_success_headers(self, data):
        try:
            return {
                "Location": reverse(
                    "uploads:api:upload-detail", kwargs={"guid": data["guid"]}
                )
            }
...
class TusPatchMixin(mixins.UpdateModelMixin):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        upload_checksum = getattr(
            request, constants.UPLOAD_CHECKSUM_FIELD_NAME, None
        )
        ...
        return Response(
            serializer.data, headers=headers, status=status.HTTP_204_NO_CONTENT
        )
