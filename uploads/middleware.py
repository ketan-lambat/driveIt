...
class TusMiddleware(object):
    ...
    @classmethod
    def parse_upload_defer_length(
        cls, request,
    ):
        upload_defer_length = cls.get_header(
            request, "Upload-Defer-Length", None
        )
        ...
        setattr(
            request,
            constants.UPLOAD_DEFER_LENGTH_FIELD_NAME,
            upload_defer_length,
        )
    ...
    def parse_upload_checksum(cls, request):
        upload_checksum_header = cls.get_header(
            request, "Upload-Checksum", None
        )
