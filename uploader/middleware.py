...
class TusMiddleware(object):
    ...
    def parse_upload_defer_length(cls, request, ):
        upload_defer_length = cls.get_header(request, 'Upload-Defer-Length',
                                             None)
        ...
        if upload_defer_length != 1:
            return HttpResponse(
                'Invalid value for "Upload-Defer-Length" header: {}.'.format(
                    upload_defer_length),
                status=status.HTTP_400_BAD_REQUEST)

        # Set upload defer length
        setattr(request, constants.UPLOAD_DEFER_LENGTH_FIELD_NAME,
                upload_defer_length)
    ...
    def parse_upload_length(cls, request):
        ...
        # Set upload length
        setattr(request, constants.UPLOAD_LENGTH_FIELD_NAME,
                int(upload_length))

    @classmethod
    def parse_upload_checksum(cls, request):
        upload_checksum_header = cls.get_header(request, 'Upload-Checksum',
                                                None)
    ...
    def parse_upload_metadata(cls, request):
        ...
        for key_value_pair in upload_meta_header.split(','):
            ...
            # Store data
            upload_metadata[key] = decode_base64(value.encode('ascii')).decode(
                'utf-8')
    ...
    def get_header(cls, request, key, default_value=None):
        ...
        # If we didn't find the key, or the value was "None", \
        # try to use the "HTTP_{uppercased-key}" key
        if result is None:
            custom_value = 'HTTP_{}'.format(key.replace('-', '_').upper())
            result = request.META.get(custom_value, default_value)

        # If we didn't find the key, or the value was "None", try to use
        # the "HTTP_X_{uppercased-key}" key
        if result is None:
            # https://tools.ietf.org/html/rfc6648
            custom_value = 'HTTP_X_{}'.format(key.replace('-', '_').upper())
            result = request.META.get(custom_value, default_value)

        # If we still didn't find the key, or the value was "None",
