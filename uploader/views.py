...
from . import tus_api_version, tus_api_version_supported, tus_api_extensions, \
...
def add_expiry_header(upload, headers):
    if upload.expires:
        headers['Upload-Expires'] = upload.expires.strftime(
            '%a, %d %b %Y %H:%M:%S %Z')
...
class UploadMetadata(BaseMetadata):
    def determine_metadata(self, request, view):
        return {
            ...
            'Tus-Max-Size': getattr(view, 'max_file_size',
                                    tus_settings.TUS_MAX_FILE_SIZE),
...
class TusHeadMixin(object):
    def info(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Instead of simply trowing a 404, we need
            # to add a cache-control header to the response
...
class TusCreateMixin(mixins.CreateModelMixin):
    def create(self, request, *args, **kwargs):
        ...
        upload_length = getattr(request, constants.UPLOAD_LENGTH_FIELD_NAME,
                                -1)
        ...
        if not upload_length or upload_length < 0:
            if getattr(request, constants.UPLOAD_DEFER_LENGTH_FIELD_NAME,
                       -1) != 1:
                return Response('Missing "{Upload-Defer-Length}" header.',
                                status=status.HTTP_400_BAD_REQUEST)

        # Get metadata from request
        upload_metadata = getattr(request,
                                  constants.UPLOAD_METADATA_FIELD_NAME, {})

        # Get data from metadata
        filename = upload_metadata.get(
            tus_settings.TUS_FILENAME_METADATA_FIELD, '')
    ...
    def get_success_headers(self, data):
        try:
            return {'Location': reverse('uploads:api:upload-detail',
                                        kwargs={'guid': data['guid']})}
...
class TusPatchMixin(mixins.UpdateModelMixin):
    ...
    def validate_chunk(self, offset, chunk_bytes):
        ...
        :param int offset:
        :param bytes chunk_bytes:
        :return bytes: The chunk_bytes
        """
        return chunk_bytes
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if upload_offset >= upload.upload_length:
            raise Conflict
